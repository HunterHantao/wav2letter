...
//  for (auto i = data[0].input.begin(); i != data[0].input.end(); ++i)
//    std::cout << *i << ' '<< std::endl;
  ...
  if (FLAGS_pow || FLAGS_mfsc || FLAGS_mfcc) {
    ...
    if (FLAGS_mfsc) {
      ...
      std::cout << "Mfsc feature size "<<featSz<<mfsc.getFeatureParams().filterFreqResponseLen() <<"feat size" <<  featSz<< std::endl;
      std::cout << "Directly frequency features, channels: " << T << std::endl;
      for (auto i = inFeat.begin(); i != inFeat.end(); ++i)
        std::cout << *i << ' '<< std::endl;
    }
    if (FLAGS_pow) {
  ...
  // HHT print feature value

  feat.sampleIdsDims = af::dim4(maxSampleIdLen, batchSz);
  std::cout << "Fianl audio frequency features " << std::endl;
  for (auto i = feat.input.begin(); i != feat.input.end(); ++i)
    std::cout << *i << ' '<< std::endl;
...
FeatureParams defineSpeechFeatureParams() {
  ...
  std::cout << "Parameter configuration "<<params.samplingFreq<< params.frameSizeMs<< params.frameStrideMs<<params.numFilterbankChans<<params.accWindow <<params.deltaWindow<< std::endl;
  //  Mfcc
